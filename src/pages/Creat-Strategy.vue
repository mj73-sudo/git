<script lang="ts" setup>
  ...
  const normalizeExitModel = () => {
    if (exitModel.value === 'tp/sl') {
      ...
      strategymodel.value.exitConditions = [];
    }
...
*/
  ...
  const validateBeforeCreate = async () => {
    strategymodel.value.conditions = [...enterConditions.value];
    strategymodel.value.exitConditions = [...exitConditions.value]
...
/*
  const validateBeforeCreate = async (index: number) => {
    ...
    // شرط ورود
    strategymodel.value.conditions = enterConditions.value.filter(cond => {
      // فقط شرط‌هایی که حداقل یکی از فیلدهای اصلی پر شده‌اند
      return cond.sourceType || cond.targetType || cond.operator;
    });

    if (strategymodel.value.conditions.length === 0) {
      alert('حداقل یک شرط ورود الزامی است.');
      return false;
    }
    ...
    const lastIndexEnter = enterConditions.value.length - 1;
    const formEnter = formConditionRefs.value[lastIndexEnter];
    const isValidEnter = await formEnter?.validate();
    if (!isValidEnter?.valid) {
      alert('فیلدهای ورود به معامله نباید خالی باشد ')
      return false;
    }

    const hasTpSl = strategymodel.value.tp != null && strategymodel.value.sl != null;

    const hasExitStrategy = exitModel.value === 'strategy' && exitConditions.value.length > 0;

    if (!hasTpSl && !hasExitStrategy) {
      alert('باید یکی از حالت‌های خروج را مشخص کنید.');
      return false;
    }
    ...
    if (hasTpSl) {
      const formValid = await tpSlForm.value?.validate();
      if (!formValid.valid) {
        alert('مقادیر TP و SL باید معتبر باشند.');
        return false;
      }
      const lastIndexExit = exitConditions.value.length - 1;
      const formExit = formExitCondition.value[lastIndexExit];
    ...
    if (hasExitStrategy) {
      ...
      const lastIndexExit = exitConditions.value.length - 1;
      const formExit = formExitCondition.value[lastIndexExit];
      const isValidExit = await formExit?.validate();
      if (!isValidExit?.valid) {
        alert('فیلدهای خروج از معامله نباید خالی باشد')
        return false;
      }
    }
    return true;
...
*/
  ...
  const create = async () => {
    ...
    normalizeExitModel();
    await strategystore.createStrategy(strategymodel.value);

    if (strategystore.error) return;
...
<template class="v-home">
  ...
  <!-- Warning strategies -->
  <v-card
    v-if="strategystore.canCreate===false"
    class="pa-4 d-flex justify-center align-center mt-5"
    flat
  >
    <v-alert
      border="top"
      ...
      variant="outlined"
    >
      <span class="alert-text--sm"> شما به سقف ایجاد استراتژی رسیده‌اید. به صفحه استراتژی‌های من هدایت می‌شوید...</span>
    ...
    >
      ...
      <v-form ref="formStep1" class="mt-4">
        <v-row class="mt-2">
          ...
          <v-col cols="12" md="4">
            <v-select
              v-model="strategymodel. symbolIds"
              class="custom"
              color="primary"
              density="comfortable"
              item-color="primary"
